...
from easy_functions import load_model, g_colab, get_mouth_landmarks_mediapipe
...
parser.add_argument(
    "--mouth_tracking_backend",
    type=str,
    default="dlib",
    choices=["dlib", "mediapipe"],
    help="Choose backend for mouth tracking: 'dlib' (default) or 'mediapipe' (faster, modern)",
)
...
def create_tracked_mask(img, original_img):
    ...
    # Detect face/mouth
    if args.mouth_tracking_backend == "mediapipe":
        mouth_points = get_mouth_landmarks_mediapipe(img)
...
def main():
    ...
    if str(args.preview_settings) == "False":
        logger.info("Converting to final video with audio")
        try:
            subprocess.check_call([
                "ffmpeg",
                "-y",
            ...
            logger.error(f"Final video conversion failed: {e}")
            raise RuntimeError(f"Failed to create final video: {e}")
        except subprocess.TimeoutExpired:
            logger.error("Final video conversion timed out")
            raise RuntimeError("Video conversion timed out after 10 minutes")
    else:
        logger.info("Preview mode completed. Check temp/preview.jpg")
...
if __name__ == "__main__":
    try:
        ...
        logger.info(f"Starting Wav2Lip inference with device: {device}")
        logger.info(f"Checkpoint: {args.checkpoint_path}")
        logger.info(f"Face input: {args.face}")
        logger.info(f"Audio input: {args.audio}")
        ...
        logger.info("Wav2Lip inference completed successfully!")
